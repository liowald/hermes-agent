...
- The factory watchdog reconciles durable phase receipts. It does not infer progress from heartbeats, process activity, or file modification times alone.
- For running coding phases, the watchdog fingerprints the current commit, Git index, relevant tracked and untracked file contents, and run-scoped durable progress events. Repeated edits to the same path therefore reset the stall clock, while heartbeat-only activity does not.
- A long read-only review records a concrete finding or completed verification step with `kanban_progress`; the event is bound to its active run and counts as semantic progress without granting repository-write access.
- If a workspace cannot be inspected completely within the bounded probe, the watcher reports an incomplete probe instead of a false stall. Monitor state advances only after the reasoning turn and requested delivery succeed, so a Buzz outage cannot silently consume the only anomaly or recovery notice.
