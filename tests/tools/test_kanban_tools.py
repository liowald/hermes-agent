...
def test_progress_is_run_bound_and_extends_claim(monkeypatch, worker_env):
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt
    ...
    try:
        ...
        conn.execute(
            "UPDATE tasks SET claim_expires=1 WHERE id=?", (worker_env,)
        )
        conn.commit()
    finally:
        conn.close()
    ...
    out = kt._handle_progress({"summary": "reviewed the storage boundary"})
    ...
    try:
        ...
        event = conn.execute(
            "SELECT kind,run_id,payload FROM task_events "
        ...
        assert "storage boundary" in event["payload"]
    finally:
        conn.close()
