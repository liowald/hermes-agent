...
def test_repo_discovery_does_not_create_or_modify_project_store(
    ...
    project_files = sorted(isolated_home.glob("projects.db*"))
    before = {
    ...
    discovered = janitor._discover_repos(None)

    after_files = sorted(isolated_home.glob("projects.db*"))
    after = {
        path.name: (path.read_bytes(), path.stat().st_mtime_ns)
        for path in after_files
    }
    assert repo.resolve() in discovered
    assert before == after


def test_repo_discovery_reads_uncheckpointed_wal_without_mutating_source(
    isolated_home, tmp_path, monkeypatch,
):
    from hermes_cli import projects_db

    repo = tmp_path / "wal-project-repo"
    repo.mkdir()
    subprocess.run(
        ["git", "init", "-b", "main"],
        cwd=repo,
    ...
    try:
        ...
        writer.executescript(
            ...
            "id INTEGER PRIMARY KEY, primary_path TEXT, archived INTEGER DEFAULT 0"
            ");"
        )
        writer.execute(
            "INSERT INTO projects(primary_path,archived) VALUES (?,0)",
            (str(repo),),
        )
        writer.commit()
        assert Path(str(db_path) + "-wal").is_file()
        monkeypatch.setattr(projects_db, "projects_db_path", lambda: db_path)
        source_files = sorted(tmp_path.glob("live-projects.db*"))
        before = {
            path.name: (path.read_bytes(), path.stat().st_mtime_ns)
        ...
        after_files = sorted(tmp_path.glob("live-projects.db*"))
        after = {
            path.name: (path.read_bytes(), path.stat().st_mtime_ns)
            for path in after_files
        }
