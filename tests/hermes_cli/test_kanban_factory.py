...
def test_factory_root_rejects_false_completion_and_requires_dual_quorum(factory_env):
    ...
    try:
        ...
        with pytest.raises(RuntimeError, match="active claimed worker run"):
            kb.complete_task(
                conn,
                reviewing["reviewer_a_task_id"],
                summary="manual approval",
                metadata={
                    "verdict": "approve",
                    "candidate_sha": candidate,
                    "findings": [],
                    "verification": ["not actually reviewed"],
                },
            )
...
def test_factory_rejects_unchanged_implementation_with_arbitrary_commit_receipt(
    factory_env,
):
    _, repo = factory_env
    conn = kb.connect()
    try:
        created = factory.create_factory(
            conn,
            title="Reject a no-op",
            body="The implementation must change the intake tree.",
            workspace_kind="dir",
            workspace_path=str(repo),
            idempotency_key="feature:reject-noop-commit-receipt",
            delivery_mode="local_commit",
        )
        head = subprocess.run(
            ["git", "rev-parse", "HEAD"],
        ...
        assert blocked["last_error"] == "implementation has no observed change from intake"
        assert blocked["reviewer_a_task_id"] is None
        assert blocked["reviewer_b_task_id"] is None
    finally:
        conn.close()


def test_reconcile_all_isolates_one_broken_root(factory_env, monkeypatch):
    _, repo = factory_env
    conn = kb.connect()
    try:
        first = factory.create_factory(
            conn,
            title="Broken root",
            body="Fail only this reconciliation.",
            workspace_kind="dir",
            workspace_path=str(repo),
            idempotency_key="feature:broken-reconcile",
            delivery_mode="local_commit",
        )
        second = factory.create_factory(
            conn,
            title="Healthy root",
            body="This root must still reconcile.",
            workspace_kind="dir",
            workspace_path=str(repo),
            idempotency_key="feature:healthy-reconcile",
            delivery_mode="local_commit",
        )
        seen = []

        def fake_reconcile(_conn, root_id):
            seen.append(root_id)
            if root_id == first["root_id"]:
                raise RuntimeError("broken workspace")
            return {"root_id": root_id, "state": "implementing"}

        monkeypatch.setattr(factory, "reconcile_factory", fake_reconcile)

        results = factory.reconcile_all(conn)

        assert seen == [first["root_id"], second["root_id"]]
        assert results[0]["state"] == "reconcile_failed"
        assert "broken workspace" in results[0]["reconcile_error"]
        assert results[1] == {
