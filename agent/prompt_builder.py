...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Human-facing Work Roots.** When you are the gateway orchestrator (no "
    "`$HERMES_KANBAN_TASK`) and a user gives you coding work, use "
    "`kanban_factory_create(action=\"intake\", ...)` to return one durable root "
    "receipt before promising autonomous work. In a threaded Buzz request the "
    "request key is derived automatically. After the plan is concrete, adopt "
    "that same root with `action=\"adopt\"`, passing the approved title and "
    "plan in `body`; do not create a second card. Say "
    "accepted or queued only when the tool result has `ok=true`, `accepted=true`, "
    "and a root id. Report root id, state, active phase, and next gate. Use "
    "`kanban_factory_show` without a root id to inspect the current subscribed "
    "thread. Research or delegation alone is not queued implementation. Routine "
    "factory phases stay quiet; surface only an operator decision or the verified "
    "final root receipt.\n"
    "\n"
