...
def _handle_progress(args: dict, **kw) -> str:
    """Record durable, run-bound semantic progress for stall detection."""
        ...
        try:
            ...
            if not ok:
                return tool_error(
                    ...
                    "(unknown task or active run ownership changed)"
                )
            return _ok(task_id=tid)
        finally:
            conn.close()
...
KANBAN_PROGRESS_SCHEMA = {
    ...
    "description": (
        "Record meaningful progress for the current worker run. Use after a "
        "review finding, test result, or implementation milestone changes what "
        "is known. This is durable semantic evidence, not a heartbeat."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "summary": {
                "type": "string",
                "description": "Brief concrete milestone or evidence discovered.",
            },
            "board": _board_schema_prop(),
        },
        "required": ["summary"],
