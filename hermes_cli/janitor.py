    ...
    if explicit is not None:
        ...
        try:
            ...
            if projects_path.is_file():
                # Read a private main+WAL copy. Direct read-only WAL access
                # still updates shared-memory lock bytes, while deserializing
                # only the main file loses committed WAL frames.
                with tempfile.TemporaryDirectory(
                    prefix="hermes-janitor-projects-"
                ) as raw:
                    snapshot = Path(raw) / projects_path.name
                    shutil.copy2(projects_path, snapshot)
                    wal = Path(str(projects_path) + "-wal")
                    if wal.is_file():
                        shutil.copy2(wal, Path(str(snapshot) + "-wal"))
                    project_conn = sqlite3.connect(snapshot)
                    try:
                        project_conn.row_factory = sqlite3.Row
                        project_conn.execute("PRAGMA query_only=ON")
                        if _table_exists(project_conn, "projects"):
                            for row in project_conn.execute(
                                ...
                                candidates.append(
                                    Path(row["primary_path"]).expanduser()
                                )
                    finally:
                        project_conn.close()
        except Exception:
