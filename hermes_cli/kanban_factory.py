...
def _create_review_task(conn, row, *, profile: str, label: str, candidate: str, bundle: str) -> str:
    ...
    body = (
        ...
        "verification: [...]}. If inspection lasts more than ten minutes, record a "
        "concrete finding or completed verification step with kanban_progress."
    )
...
def _promote_reviews(conn, row, receipt: dict) -> dict[str, Any]:
    ...
    if not observed:
        return _set_error(conn, row["root_id"], "implementation has no observed change from intake")
...
def reconcile_all(conn) -> list[dict[str, Any]]:
    ...
    for root in roots:
        try:
            results.append(reconcile_factory(conn, root))
        except Exception as exc:
            results.append({
                "root_id": root,
                "state": "reconcile_failed",
                "reconcile_error": f"{type(exc).__name__}: {exc}"[:2000],
            })
    return results
